...
ProcessResult run_command(const std::vector<std::string> &command,
  ...
  if (!stdin_data.empty()) {
    DWORD written;
    WriteFile(inWr, stdin_data.data(), (DWORD)stdin_data.size(), &written,
              nullptr);
  }
  CloseHandle(inWr);
  ...
  while (true) {
    ...
    // Drain stdout
    DWORD avail = 0;
    while (PeekNamedPipe(outRd, nullptr, 0, nullptr, &avail, nullptr) &&
           avail > 0) {
      DWORD nread;
      if (!ReadFile(outRd, buffer, min<DWORD>((DWORD)sizeof(buffer), avail),
                    &nread, nullptr))
        break;
    ...
    // Drain stderr
    while (PeekNamedPipe(errRd, nullptr, 0, nullptr, &avail, nullptr) &&
           avail > 0) {
      DWORD nread;
      if (!ReadFile(errRd, buffer, min<DWORD>((DWORD)sizeof(buffer), avail),
                    &nread, nullptr))
        break;
  ...
  while (!finished) {
    ...
    select(std::max(stdout_pipe[0], stderr_pipe[0]) + 1, &fds, nullptr, nullptr,
           &tv);
