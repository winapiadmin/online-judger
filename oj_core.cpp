...
#ifdef _WIN32
BOOL WINAPI SignalHandler(DWORD){ fn(); return TRUE; }
#else
void SignalHandler(int v){ if (v==SIGINT) {fn(); exit(0); } }
#endif
void termination() {
...
int main(int argc, char **argv) {
  ...
  io->add_option("-s,--submissions", subdir)->required()->option_text("PATH")->check(CLI::ExistingDirectory);
  io->add_option("-t,--tests", tdir)->required()->option_text("PATH")->check(CLI::ExistingDirectory);

  auto *cfg = app.add_option_group("Configuration");
  cfg->add_option("-c,--settings", compfile)->option_text("FILE")->check(CLI::ExistingFile);
  cfg->add_option("-j,--judge-paths", judgers)->option_text("PATH")->check(CLI::ExistingDirectory);
  ...
  if (!compfile.empty())compfile=fs::canonical(compfile);
  ...
  if (!compfile.empty())
  {
    ...
    {
      throw std::runtime_error("Is this excessive for compiler info? Possibly specifically crafted input.");
    }
  ...
  for (auto& fd:fs::directory_iterator(tdir)){
    ...
    if (fs::exists(settings_path)){
      ...
      binary.seekg(0, ios::beg);
      if (size>(1<<24)) // 16MiB of testcases even if uncompressed?
        throw std::runtime_error("Is this excessive for test cases? Possibly specifically crafted input. please re-check config");
      std::vector<char> data(size);
      binary.read(data.data(), size);
      uLongf destSize = size * 1032; // starting guess
      std::vector<char> processed(destSize);
      ...
      if (rc==Z_OK){
        ...
        parseSettingsFormat(std::string_view(processed.data(), destSize), testcases[name]);
      }
      else{
        PLOGD <<settings_path.string()<<" is not ZLIB compressed, error="<<rc;
        parseSettingsFormat(std::string_view(data.data(), data.size()), testcases[name]);
      }
    }
    else{
  ...
  for (auto& user:fs::directory_iterator(subdir)){
    ...
    for (auto &problem:testcases){
      judge(subdir, tdir, problem.first, user.path().stem().string(), globalInfo, testcases, judgers);
    }
  }
  auto print_stats=[&](){
  ...
  if (!waitSubmittorMode){
    ...
    auto callback_judge=[&](fs::path path) -> void {
        judge(subdir, tdir, path.filename().stem().string(), path.parent_path().filename().string(), globalInfo, testcases, judgers);
        print_stats();
    };
